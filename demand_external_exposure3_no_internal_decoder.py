...
- external predicted exposure hats are appended as future covariates
- no internal exposure decoder; no true future DPH is used as demand input
...
def load_real_data(data_raw, dph_cap_q=0.995):
    ...
    print("External exposure safe mode: demand uses predicted DPH hats only")
...
def attach_external_exposure3_to_raw_data(
    ...
    exposure_mode:
      ...
      "buybox_only":
          use only buy_box hat

      "total_instock":
          use total + instock hats

      "buybox_instock":
          use buy_box + instock hats
...
USAGE IN JUPYTER
----------------
%run -i demand_external_exposure3_clean.py
...
# Recommended: all three DPH hats
...
# Optional comparison: in-stock only
