...
def prepare_instock_chronos2_df(
    ...
    for c in ["in_stock_dph", "fbi_demand", "total_dph", "buy_box_dph", "glance_view_count"]:
        if c in df.columns:
            df[c] = _safe_numeric(df[c]).clip(lower=0.0)
    ...
    for c in ["fbi_demand", "total_dph", "buy_box_dph"]:
        if c not in df.columns:
            df[c] = 0.0
        df[c] = _safe_numeric(df[c]).clip(lower=0.0)
    ...
    if "fbi_demand" in df.columns:
        past_covariates_list.append("fbi_demand")
    if use_total_buybox_as_past:
        for c in ["total_dph", "buy_box_dph"]:
            if c in df.columns:
                past_covariates_list.append(c)
    if use_glance_view_count and "glance_view_count" in df.columns:
        past_covariates_list.append("glance_view_count")

    past_covariates_list += existing_demand_style_cols
    past_covariates_list = list(dict.fromkeys([c for c in past_covariates_list if c != "in_stock_dph"]))

    for c in known_covariates_list + past_covariates_list:
        if c not in df.columns:
            df[c] = 0.0
        df[c] = _safe_numeric(df[c])

    print("=" * 100)
    print("Chronos data ready: target=in_stock_dph")
    print(f"ASINs: {df['asin'].nunique()} | Rows: {len(df)}")
    print(f"Known future covariates: {len(known_covariates_list)}")
    print(f"Past-only covariates, no future leakage: {len(past_covariates_list)}")
    print("Past-only covariates:", past_covariates_list)
    print(f"Target cap: {cap}")
    print("=" * 100)

    return df, known_covariates_list, past_covariates_list
...
def build_instock_chronos2_train_future(
    ...
    past_covariates_list = past_covariates_list or []

    use_cols = ["asin", "order_week", target_col] + known_covariates_list + past_covariates_list
    ...
    print(f"Known future covariates passed to predict: {len(known_covariates_list)}")
    print(f"Past-only covariates available only in train history: {len(past_covariates_list)}")
    print("=" * 100)
...
def run_full_chronos_tcn_experiment(
    ...
    chronos_df, known_covariates_list, past_covariates_list = prepare_instock_chronos2_df(
        data_raw1=data_raw1,
        scot_df=scot_df,
        n_asins=n_asins,
        seed=seed,
...
if RUN_EXPERIMENT_NOW:
    result = run_full_chronos_tcn_experiment(
        ...
        history=HISTORY,
        horizon=HORIZON,
        ag_time_limit=CHRONOS_TIME_LIMIT,
        fine_tune_steps=FINE_TUNE_STEPS,
        fine_tune_lr=FINE_TUNE_LR,
    ...
    try:
        ...
        display(compare_df[[
            ...
            "true_instock_dph", "pred_instock_dph", "chronos_pred_instock_dph",
            "tcn_abs_err_instock", "chronos_abs_err_instock", "chronos_better"
        ]].head(50))
    except Exception:
        print(summary.round(5).to_string(index=False))
        print(by_horizon.round(5).to_string(index=False))
