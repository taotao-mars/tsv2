...
# ============================================================
# 14. TFT-STYLE EXPOSURE MODEL FOR COMPARISON
# ============================================================
# Goal:
#   Compare current best anchor-attention exposure model vs a TFT-style model.
#
# This model predicts:
#   total_dph, buy_box_dph, in_stock_dph
#
# It uses:
#   - history sequence x
#   - known future_context, including explicit event features
#   - multi-horizon decoder
#   - variable selection
#   - LSTM encoder/decoder
#   - horizon self-attention
#
# Important:
#   This is a lightweight PyTorch implementation.
#   No pytorch-forecasting package is required.
# ============================================================


class GRN(nn.Module):
    """
    Gated Residual Network block used in TFT-style models.
    """
    def __init__(self, input_dim, hidden_dim, output_dim=None, dropout=0.10):
        super().__init__()
...
class VariableSelectionNetwork(nn.Module):
    """
    Lightweight variable selection.

    Input:
      x: [B,T,F]

    Output:
      selected representation: [B,T,D]
      weights: [B,T,F]
    """
    ...
    def forward(self, x):
        ...
        for j in range(self.n_vars):
            embs.append(self.var_proj[j](x[:, :, j:j+1]))
        embs = torch.stack(embs, dim=-2)  # [B,T,F,D]

        # This keeps the [B,T,F] shape. Variable selection is per time step,
        # then the LSTM models temporal dependence across T.
...
class TFTStyleExposureModel(nn.Module):
    """
    TFT-style model for multi-output exposure forecasting.

    It predicts log1p levels:
      output[...,0] = log1p(total_dph_hat)
      output[...,1] = log1p(buy_box_dph_hat)
      output[...,2] = log1p(in_stock_dph_hat)
    """
    def __init__(
        self,
        input_dim,
        ...
        self.head = nn.Sequential(
            nn.Linear(d_model, max(64, d_model)),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(max(64, d_model), 3),
        )
    ...
    def _horizon_features(self, B, H, device, dtype):
        ...
        feats = torch.stack([
            h_norm,
            torch.sin(2 * np.pi * h_norm),
            torch.cos(2 * np.pi * h_norm),
            (h >= 13).float(),
        ], dim=-1)
    ...
    def forward(self, x, future_context, return_aux=False):
        ...
        dec_in = torch.cat([fut_sel, h_feat], dim=-1)
        dec_out, _ = self.decoder_lstm(dec_in, (h_n, c_n))

        # Cross-attention:
        #   Query  = future horizon decoder states [B,H,D]
        #   Key    = full historical encoder states [B,T,D]
        #   Value  = full historical encoder states [B,T,D]
        #
        # This is more suitable for direct multi-horizon forecasting:
        # every horizon h can directly attend to the whole 52-week history.
        ...
        z = self.post_attn(dec_out + attn_out)

        # Predict log1p level.
        log_hat = F.softplus(self.head(z))
...
def tft_exposure_loss(
    ...
    """
    Loss designed for your current problem:
      - log-level Huber
      - mean scale penalty
      - underbias penalty
      - high exposure weighting
    ...
    # Give more weight to high true exposure rows.
    denom = target_log.detach().mean(dim=(0, 1), keepdim=True).clamp_min(1e-6)
    high_w = 1.0 + high_weight_alpha * target_log.detach() / denom

    # Give slightly more weight to longer horizon.
    H = true.shape[1]
    h = torch.arange(1, H + 1, device=true.device, dtype=true.dtype).view(1, H, 1)
    horizon_w = 1.0 + horizon_weight_alpha * (h / max(float(H), 1.0))

    weight = target_weights * high_w * horizon_w

    point = F.huber_loss(pred_log, target_log, delta=1.0, reduction="none")
    point = (point * weight).mean()

    mean_pred = torch.log1p(pred_level.mean(dim=(0, 1)))
    mean_true = torch.log1p(true.mean(dim=(0, 1)).clamp_min(1e-6))
    mean_loss = torch.mean(torch.abs(mean_pred - mean_true) * target_weights.view(3))

    # Underbias penalty only on positive-true exposure cells.
    # For true=0, under is already zero, but explicitly masking positives
    # keeps the penalty focused on missed magnitude instead of zero-heavy rows.
    under = torch.relu(true - pred_level)
    positive_mask = (true > 0).float()
    under_loss = (
        weight
        * positive_mask
        * under
        ...
        (weight * positive_mask).sum().clamp_min(1.0)
    )

    return point + mean_weight * mean_loss + under_weight * under_loss


def train_tft_exposure_model(
    model,
    tr_ld,
    va_ld,
    epochs=60,
    ...
    for epoch in range(epochs):
        ...
        print(f"[TFT] Epoch {epoch+1:03d} | train={tr_loss:.5f} | val={va_loss:.5f}")
        ...
        if no_improve >= patience:
            print(f"[TFT] Early stop at epoch {epoch+1}. Best val={best_val:.5f}")
            break
...
def predict_tft_exposure(model, va_ld, apply_funnel_constraint=True):
    rows = []
    model.eval()
    ...
    with torch.no_grad():
        for b in va_ld:
            log_hat = model(b["x"], b["future_context"])
            pred = torch.expm1(log_hat).clamp(min=0.0).cpu().numpy()
...
def make_external_hat_df_from_tft(pred_df):
    """
    Create exposure_hat_for_demand from TFT predictions.
    """
    out = pred_df[
        [
            "asin",
...
def run_tft_exposure_forecast(
    ...
    """
    Main TFT-style exposure runner.
    Uses the same data split and event-feature future_context as the best-attn pipeline.
    """
    print("\n" + "=" * 100)
    print("TFT-STYLE EXPOSURE FORECAST")
    print("=" * 100)
    ...
    exposure_hat_for_demand_tft = make_external_hat_df_from_tft(pred_df)
    ...
    return {
        ...
        "context_dim": context_dim,
        "data": data,
        "exposure_hat_for_demand": exposure_hat_for_demand_tft,
    }


def compare_best_attn_vs_tft_exposure(result_best, result_tft):
    """
    Compact comparison of best-attn vs TFT on exposure metrics.
    """
    best_overall = result_best["attn_eval"]["quality"]["overall"].copy()
    best_overall.insert(0, "method", "best_attn")

    tft_overall = result_tft["exposure_quality"]["overall"].copy()
    tft_overall.insert(0, "method", "tft_style")

    overall = pd.concat([best_overall, tft_overall], ignore_index=True)

    print("\n" + "=" * 100)
    print("BEST ATTN VS TFT: OVERALL EXPOSURE QUALITY")
    print("=" * 100)
    print(overall.round(5).to_string(index=False))

    best_h = result_best["attn_eval"]["quality"]["by_horizon"].copy()
    best_h.insert(0, "method", "best_attn")

    tft_h = result_tft["exposure_quality"]["by_horizon"].copy()
    tft_h.insert(0, "method", "tft_style")

    by_h = pd.concat([best_h, tft_h], ignore_index=True)

    print("\n" + "=" * 100)
    print("BEST ATTN VS TFT: IN_STOCK BY HORIZON")
    print("=" * 100)

    show = by_h[by_h["target"] == "in_stock_dph"].copy()
...
# ============================================================
# This runs:
#   1. current best anchor-attention exposure model
#   2. TFT-style exposure model
#   3. comparison vs real DPH inside exposure diagnostics
#
# Downstream demand model can then use either:
#   exposure_hat_for_demand_best_attn
#   exposure_hat_for_demand_tft
# ============================================================

# 1. Current best-attn exposure model
result_best = run_best_exposure_anchor_attention(
    data_raw1=data_raw1,
    scot_df=scot_df,
    n_asins=5000,
...
exposure_hat_for_demand_best_attn = result_best["exposure_hat_for_demand"]

# 2. TFT-style exposure model
result_tft = run_tft_exposure_forecast(
    data_raw1=data_raw1,
    scot_df=scot_df,
    n_asins=5000,
    seed=42,
...
exposure_hat_for_demand_tft = result_tft["exposure_hat_for_demand"]

# 3. Compare exposure quality
comparison_best_attn_vs_tft = compare_best_attn_vs_tft_exposure(
    result_best=result_best,
    result_tft=result_tft,
)

# Compatibility alias:
# Choose which one you want to feed into demand model.
# Option A:
exposure_hat_for_demand_calib = exposure_hat_for_demand_best_attn

# Option B:
# exposure_hat_for_demand_calib = exposure_hat_for_demand_tft

print("\n" + "=" * 100)
print("READY FOR DEMAND MODEL")
print("=" * 100)
print("Best-attn exposure hat:")
print("  exposure_hat_for_demand_best_attn")
print("TFT exposure hat:")
print("  exposure_hat_for_demand_tft")
print("\nCurrently selected alias:")
print("  exposure_hat_for_demand_calib = exposure_hat_for_demand_best_attn")
