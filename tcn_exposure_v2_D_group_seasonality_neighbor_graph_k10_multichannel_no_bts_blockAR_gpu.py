...
class TCNDecoderWithCrossAttn(nn.Module):
    ...
    def __init__(self, d_model, context_dim, horizon=20,
        ...
        self.use_block_ar = bool(use_block_ar)
        self.block_ar_scale = float(block_ar_scale)
        ...
        # Block semi-AR refinement.
        # Direct head first predicts all 20 horizons. Then:
        #   h=6..12  is refined using summary(pred h=1..5)
        #   h=13..20 is refined using summary(pred h=1..5, h=6..12)
        # This is not full AR, so it avoids step-by-step error accumulation.
        # Summary per block = mean, last, max, slope for 3 channels => 12 dims.
        block_mid_in = d_model + z_extra + 12
        block_long_in = d_model + z_extra + 24
        self.block_mid_head = nn.Sequential(
            nn.Linear(block_mid_in, hidden),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(hidden, hidden // 2),
            nn.ReLU(),
        ...
        self.block_long_head = nn.Sequential(
            nn.Linear(block_long_in, hidden),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(hidden, hidden // 2),
            nn.ReLU(),
    ...
    @staticmethod
    def _block_summary(log_level_block):
        # log_level_block: [B, L, 3]
        mean = log_level_block.mean(dim=1)
        last = log_level_block[:, -1, :]
        maxv = log_level_block.max(dim=1).values
        slope = log_level_block[:, -1, :] - log_level_block[:, 0, :]
    ...
    def forward(self, enc_out, future_context, return_aux=False, z=None):
        ...
        raw_log_mag_base = raw_log_mag
        raw_log_mag = self._apply_block_ar_refinement(raw_log_mag, z_out, z_rep)
        ...
        if return_aux:
            ...
            return {
                ...
                "gate": gate,
                "residual": residual,
                "raw_log_mag_base": raw_log_mag_base,
                "use_block_ar": self.use_block_ar,
                "block_ar_scale": self.block_ar_scale,
                "z": z,
                "attn_weights": attn_w,
            }
        return log_hat
...
class ExposureForecastModelV2(nn.Module):
    ...
    def __init__(self, input_dim, context_dim,
                 ...
                 use_block_ar=True, block_ar_scale=0.35):
        super().__init__()
        self.use_enn = use_enn
        self.z_dim = int(z_dim)
        print(f"Exposure ENN regime enabled: {use_enn} | z_dim={z_dim}")
        print(f"Block-AR refinement enabled: {use_block_ar} | scale={block_ar_scale}")
        ...
        self.decoder = TCNDecoderWithCrossAttn(
            ...
            use_block_ar=use_block_ar,
            block_ar_scale=block_ar_scale,
        )
...
def run_exposure_v2(
    ...
    print("\n" + "=" * 100)
    print("EXPOSURE MODEL V2: SINGLE-HEAD DIRECT + GROUP SEASONALITY + MULTI-CHANNEL NEIGHBOR GRAPH + BLOCK-AR")
    print("=" * 100)
