...
# ============================================================
# Buy-box focused graph / neighbor feature builder
# ============================================================
...
def _spell_stats(active):
    active = np.asarray(active).astype(bool)
    if len(active) == 0:
        return 0.0, 0.0, 0.0, 0.0

    active_runs, zero_runs = [], []
    cur = bool(active[0])
    length = 1
    for v in active[1:]:
        v = bool(v)
    ...
    avg_zero = float(np.mean(zero_runs)) if zero_runs else 0.0

    # transition rates per adjacent pair
    if len(active) <= 1:
        a2z = 0.0
        z2a = 0.0
    else:
        prev = active[:-1]
        nxt = active[1:]
        a_denom = max(prev.sum(), 1)
        z_denom = max((~prev).sum(), 1)
        a2z = float(((prev) & (~nxt)).sum() / a_denom)
        z2a = float(((~prev) & (nxt)).sum() / z_denom)

    return avg_active, avg_zero, a2z, z2a


def _build_buybox_graph_feature_table(df, history_end_week=None, k=20, min_rows_per_asin=4):
    """
    Build ASIN-level KNN graph features centered on buy_box behavior.

    Important: features are computed only from rows before history_end_week when provided,
    so the single-window validation horizon is not leaked into graph summaries.
    """
    hist = df.copy()
    if history_end_week is not None:
        hist = hist[hist["order_week"] < pd.to_datetime(history_end_week)].copy()

    if hist.empty or hist["asin"].nunique() < 3:
        return pd.DataFrame({"asin": df["asin"].astype(str).unique()}), []

    rows = []
    for asin, g in hist.groupby("asin"):
        g = g.sort_values("order_week")
        if len(g) < min_rows_per_asin:
            continue

        buy = _safe_numeric(g["buy_box_dph"]).clip(lower=0.0).values.astype(float)
        demand = _safe_numeric(g.get("fbi_demand", 0.0)).clip(lower=0.0).values.astype(float)
        oos = _safe_numeric(g.get("scot_oos", 0.0)).clip(0, 1).values.astype(float)
        promo = _safe_numeric(g.get("ind_promotion", 0.0)).clip(0, 1).values.astype(float)
        price = _safe_numeric(g.get("our_price", 0.0)).clip(lower=0.0).values.astype(float)

        active = buy > 0
        avg_active_spell, avg_zero_spell, a2z, z2a = _spell_stats(active)
        mean = float(np.mean(buy))
        q90 = float(np.quantile(buy, 0.90)) if len(buy) else 0.0
        q95 = float(np.quantile(buy, 0.95)) if len(buy) else 0.0
        mx = float(np.max(buy)) if len(buy) else 0.0
        cv = float(np.std(buy) / (mean + 1e-8)) if mean > 0 else 0.0
        topk = max(1, int(np.ceil(0.20 * len(buy))))
        top20_share = float(np.sort(buy)[-topk:].sum() / (buy.sum() + 1e-8)) if buy.sum() > 0 else 0.0

        # last streak and weeks since positive are useful transition signals.
        if len(active) > 0:
            last_state = active[-1]
            streak = 0
            for v in active[::-1]:
            ...
            weeks_since_pos = float(len(active) - 1 - pos_idx[-1]) if len(pos_idx) else float(len(active))
        else:
            streak = 0.0
            weeks_since_pos = 0.0
        ...
        rows.append({
            "asin": str(asin),
            "gl_product_group_raw": str(g["gl_product_group"].iloc[0]) if "gl_product_group" in g.columns else "MISSING",
            "category_code_raw": str(g["category_code"].iloc[0]) if "category_code" in g.columns else "MISSING",
            "bb_mean_log": np.log1p(mean),
            "bb_q90_log": np.log1p(q90),
            "bb_q95_log": np.log1p(q95),
            "bb_max_log": np.log1p(mx),
            "bb_zero_rate": float((buy <= 0).mean()),
            "bb_active_rate": float(active.mean()),
            "bb_cv_log": np.log1p(min(cv, 50.0)),
            "bb_gini": _gini_1d(buy),
            "bb_top20_share": top20_share,
            "bb_max_over_mean_log": np.log1p(mx / (mean + 1e-8)) if mean > 0 else 0.0,
            "bb_active_to_zero_rate": a2z,
            "bb_zero_to_active_rate": z2a,
            "bb_avg_active_spell_log": np.log1p(avg_active_spell),
            "bb_avg_zero_spell_log": np.log1p(avg_zero_spell),
            "bb_last_active_streak_log": np.log1p(streak if last_state else 0.0),
            "bb_last_zero_streak_log": np.log1p(streak if not last_state else 0.0),
            "bb_weeks_since_positive_log": np.log1p(weeks_since_pos),
            "demand_active_rate": float((demand > 0).mean()),
            "promo_rate": float(np.mean(promo)) if len(promo) else 0.0,
            "oos_rate": float(np.mean(oos)) if len(oos) else 0.0,
            "price_log_mean": float(np.mean(np.log1p(price))) if len(price) else 0.0,
        })

    stats = pd.DataFrame(rows)
    all_asins = pd.DataFrame({"asin": df["asin"].astype(str).unique()})
    if stats.empty or len(stats) < 3:
        return all_asins, []

    # Factorized category/GL enter KNN only as weak regime identifiers.
    stats["gl_code_knn"] = pd.factorize(stats["gl_product_group_raw"].fillna("MISSING"))[0].astype(float)
    stats["cat_code_knn"] = pd.factorize(stats["category_code_raw"].fillna("MISSING"))[0].astype(float)

    knn_cols = [
        "bb_mean_log", "bb_q95_log", "bb_zero_rate", "bb_active_rate",
        "bb_cv_log", "bb_gini", "bb_top20_share", "bb_max_over_mean_log",
        "bb_active_to_zero_rate", "bb_zero_to_active_rate",
        "bb_avg_active_spell_log", "bb_avg_zero_spell_log", "bb_weeks_since_positive_log",
        "demand_active_rate", "promo_rate", "oos_rate", "price_log_mean",
        "gl_code_knn", "cat_code_knn",
    ]
    agg_cols = [
        "bb_mean_log", "bb_q90_log", "bb_q95_log", "bb_max_log",
        "bb_zero_rate", "bb_active_rate", "bb_cv_log", "bb_gini", "bb_top20_share", "bb_max_over_mean_log",
        "bb_active_to_zero_rate", "bb_zero_to_active_rate",
        "bb_avg_active_spell_log", "bb_avg_zero_spell_log",
        "bb_last_active_streak_log", "bb_last_zero_streak_log", "bb_weeks_since_positive_log",
        "demand_active_rate", "promo_rate", "oos_rate", "price_log_mean",
    ]

    X = stats[knn_cols].replace([np.inf, -np.inf], np.nan).fillna(0.0).values.astype(float)
    X = StandardScaler().fit_transform(X)
    k_eff = int(min(max(1, k), len(stats) - 1))
    nn = NearestNeighbors(n_neighbors=k_eff + 1, metric="euclidean")
    nn.fit(X)
    dist, idx = nn.kneighbors(X, return_distance=True)

    vals = stats[agg_cols].values.astype(float)
    nb = []
    for i in range(len(stats)):
        neigh_idx = idx[i, 1:]
        neigh_dist = dist[i, 1:]
        w = 1.0 / (neigh_dist + 1e-3)
        w = w / (w.sum() + 1e-12)
        agg = (vals[neigh_idx] * w[:, None]).sum(axis=0)
        row = {"asin": stats["asin"].iloc[i]}
        for c, v in zip(agg_cols, agg):
            row[f"graph_buybox_neighbor_{c}"] = float(v)
        row["graph_buybox_neighbor_count"] = float(k_eff)
        nb.append(row)

    nb = pd.DataFrame(nb)
    graph_cols = [c for c in nb.columns if c != "asin"]

    # Standardize graph columns for stable neural input. Rates are also standardized; this is intentional.
    for c in graph_cols:
        mu = nb[c].replace([np.inf, -np.inf], np.nan).mean()
        sd = nb[c].replace([np.inf, -np.inf], np.nan).std()
        if not np.isfinite(sd) or sd < 1e-8:
            nb[c] = 0.0
        else:
            nb[c] = (nb[c].replace([np.inf, -np.inf], np.nan).fillna(mu) - mu) / (sd + 1e-8)
            nb[c] = nb[c].clip(-5.0, 5.0)

    out = all_asins.merge(nb, on="asin", how="left")
    for c in graph_cols:
        out[c] = out[c].fillna(0.0).astype(float)

    return out, graph_cols

def load_exposure_data(data_raw, dph_cap_q=0.995, use_buybox_graph_features=False, graph_k=20, graph_history_end_week=None):
    df = data_raw.copy()
    df["asin"] = df["asin"].astype(str)
    df["order_week"] = pd.to_datetime(df["order_week"])
    ...
    graph_cols = []
    if use_buybox_graph_features:
        graph_tbl, graph_cols = _build_buybox_graph_feature_table(
            df,
            history_end_week=graph_history_end_week,
            k=graph_k,
        )
        df = df.merge(graph_tbl, on="asin", how="left")
        for c in graph_cols:
            df[c] = _safe_numeric(df[c], fill=0.0)
        print(f"Buy-box graph features enabled: k={graph_k} | graph_dim={len(graph_cols)} | history_end={graph_history_end_week}")
    ...
    context_cols = list(dict.fromkeys(
        ...
        + static_cols
        # ── Buy-box KNN graph neighbor features（static, ASIN-level）────
        + graph_cols
        # ── 历史anchor──────────────────────────────────────
        + [
    ...
    for asin, g in df.groupby("asin"):
        ...
        if len(graph_cols) > 0:
            graph_matrix = g[graph_cols].values.astype(np.float32)
            features = np.concatenate([base_features, graph_matrix], axis=1).astype(np.float32)
        else:
    ...
    if len(graph_cols) > 0:
        print(f"Graph context cols added: {len(graph_cols)}")
...
def run_exposure_v2(
    ...
    use_buybox_graph_features=True,
    graph_k=20,
):
    print("\n" + "=" * 100)
    print("EXPOSURE MODEL V2: TCN Full-Seq Encoder + Cross-Attn + SINGLE-HEAD DIRECT + BUYBOX GRAPH FEATURES")
    ...
    # For single-window validation, build graph features using only weeks before the validation horizon.
    _weeks = np.sort(pd.to_datetime(df["order_week"]).dropna().unique())
    graph_history_end_week = _weeks[-horizon] if len(_weeks) >= horizon else None
    data, context_dim, context_cols = load_exposure_data(
        df,
        dph_cap_q=dph_cap_q,
        use_buybox_graph_features=use_buybox_graph_features,
        graph_k=graph_k,
        graph_history_end_week=graph_history_end_week,
    )
...
def run_exposure_v2(
    ...
    print("\n" + "=" * 100)
    print("EXPOSURE MODEL V2: SINGLE-HEAD DIRECT + SCOT OPTION + BUYBOX GRAPH FEATURES")
    print("=" * 100)
...
def run_exposure_v2_rolling(
    ...
    print("\n" + "=" * 100)
    print("EXPOSURE MODEL V2: ROLLING BACKTEST + SCOT INTERSECTION + BUYBOX GRAPH FEATURES")
    print("=" * 100)
